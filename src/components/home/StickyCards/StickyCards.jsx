...
export default function StickyCards() {
    ...
    { text: "Projects", className: "text-[13vw] md:text-[10vw] lg:text-[8vw]"}
  ];
    // We added an empty card at the start to act as the placeholder for our header
    const stickyCardsData = [
        { isHeader: true },
        { index: "01", title: "Buyzzie", image: "/assets/images/sticky-cards/card-1.jpg", description: "Your description..." },
        { index: "02", title: "My Portfolio Website", image: "/assets/images/sticky-cards/card-2.jpg", description: "From soft gradients to hand edges..." },
        ...
        <div className="sticky-cards" ref={container}>
            {stickyCardsData.map((cardData, index) => (
                <div className="sticky-card" key={index}>
                    ...
                    {/* CONDITION: If it's the first item, show the header */}
                    {index === 0 ? (
                        <div className="flex items-center justify-center w-full h-full">
                            <AnimatedHeader 
                                eyebrow="What I build with my skills" 
                                lines={headerLines} 
                            />
                        </div>
                    ) : (
                        /* OTHERWISE: Show the standard card layout */
                        <>
                            <div className="sticky-card-index">
                                <h1 className="text-8xl">{cardData.index}</h1>
                            </div>
                            <div className="sticky-card-content">
                                <div className="sticky-card-content-wrapper">
                                    <h1 className="sticky-card-header text-6xl">{cardData.title}</h1>

                                    <div className="sticky-card-img">
                                        <img className="object-cover" src={cardData.image} alt={cardData.title} />
                                    </div>
